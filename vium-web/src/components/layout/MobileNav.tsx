...
import { Zap, MapPin, Coins, UserCircle } from 'lucide-react';
...
export const MobileNav: React.FC<MobileNavProps> = ({ onOpenMyPage }) => {
  ...
  return (
    <nav className="md:hidden bg-white/90 backdrop-blur-xl border-t border-gray-100 fixed bottom-0 w-full flex justify-around py-4 px-2 z-40">
      ...
      <button 
        ...
        className={`flex flex-col items-center gap-1 ${isAuthenticated ? 'text-gray-600' : 'text-gray-300'}`}
      >
        <UserCircle size={22} />
